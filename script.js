...
(function init() {
  ...
  if (STATE.apiKey) {
    ...
    setStatus('AWAITING ACTIVATION', 'standby');
  }
})();
...
async function runBuiltIn(q) {
  ...
  // Music / Play YouTube
  if (/play (.+) on youtube|play (.+)/i.test(q)) {
    const match = q.match(/play (.+?)(?:\s+on youtube)?$/i);
    if (match) {
      const song = encodeURIComponent(match[1].replace('on youtube','').trim());
      window.open(`https://www.youtube.com/results?search_query=${song}`, '_blank');
      return `Playing ${match[1].replace('on youtube','').trim()} on YouTube, sir.`;
    }
  }
